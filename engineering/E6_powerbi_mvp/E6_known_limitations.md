...
| L4 | **Live Desktop refresh pending** | the running `.pbix` model had an uncommitted MCP transaction (empty-string type mismatch, now fixed in TMDL); real in-model refresh + row counts to be confirmed | discard the pending change in Desktop, import the corrected `V1/PBI/tmdl/`, set `DriftDataFolder`, refresh (M now nulls empty strings) |
