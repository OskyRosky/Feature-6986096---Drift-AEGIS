# push-e7d6-volatility.ps1
# E7D.6 - Publish ONLY the AEGIS Forecast Drift "Volatility" dashboard
# (uid aegis-forecast-drift-volatility) into the governed Grafana folder via HTTP API.
#
# SECURITY:
...
#   - Refuses to publish if the Volatility UID currently lives OUTSIDE the AEGIS folder.
#   - Idempotent: updates the same UID in place (no duplicate, no new dashboard).
#   - Touches ONLY the Volatility dashboard. Does not modify the Overview, Forecast,
#     Performance, Shape, Stability, the other 5 section shells, datasources, nginx,
#     Docker, CSVs, Python, Power BI, alerts, or the token.

[CmdletBinding()]
param(
    [string]$DashboardsDir = (Join-Path $PSScriptRoot '..\grafana\dashboards'),
    [string]$Uid           = 'aegis-forecast-drift-volatility',
    ...
    [string]$Message       = 'E7D.6 Volatility MVP — analytical dashboard (avg/max volatility drift score, computable count, coverage 85.7%, drift trend, ranking barchart, signal details, governed auxiliary metrics, non-computable summary, run, DQ)'
)

$ErrorActionPreference = 'Stop'
function Info([string]$m) { Write-Host "[push-e7d6] $m" }
function Die([string]$m)  { Write-Host "[push-e7d6] ERROR: $m" -ForegroundColor Red; exit 1 }

if (-not (Test-Path -LiteralPath $TokenPath)) { Die "Encrypted token not found at $TokenPath." }
$file = Join-Path $DashboardsDir "$Uid.json"
if (-not (Test-Path -LiteralPath $file)) { Die "Volatility dashboard file not found: $file" }
...
try {
    ...
    # --- 1. Conflict guard: Volatility UID must not live in a foreign folder ---
