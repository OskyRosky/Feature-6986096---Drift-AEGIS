# push-e7d4-shape.ps1
# E7D.4 - Publish ONLY the AEGIS Forecast Drift "Shape" dashboard
# (uid aegis-forecast-drift-shape) into the governed Grafana folder via HTTP API.
#
# SECURITY:
...
#   - Refuses to publish if the Shape UID currently lives OUTSIDE the AEGIS folder.
#   - Idempotent: updates the same UID in place (no duplicate, no new dashboard).
#   - Touches ONLY the Shape dashboard. Does not modify the Overview, Forecast,
#     Performance, the other 7 section shells, datasources, nginx, Docker, CSVs,
#     Python, Power BI, alerts, or the token.

[CmdletBinding()]
param(
    [string]$DashboardsDir = (Join-Path $PSScriptRoot '..\grafana\dashboards'),
    [string]$Uid           = 'aegis-forecast-drift-shape',
    ...
    [string]$Message       = 'E7D.4 Shape MVP — analytical dashboard (avg shape drift score, computable count, coverage, max score, drift trend, signal details, run, DQ)'
)

$ErrorActionPreference = 'Stop'
function Info([string]$m) { Write-Host "[push-e7d4] $m" }
function Die([string]$m)  { Write-Host "[push-e7d4] ERROR: $m" -ForegroundColor Red; exit 1 }

if (-not (Test-Path -LiteralPath $TokenPath)) { Die "Encrypted token not found at $TokenPath." }
$file = Join-Path $DashboardsDir "$Uid.json"
if (-not (Test-Path -LiteralPath $file)) { Die "Shape dashboard file not found: $file" }
...
try {
    ...
    # --- 1. Conflict guard: Shape UID must not live in a foreign folder ---
