# push-e7d3-performance.ps1
# E7D.3 - Publish ONLY the AEGIS Forecast Drift "Performance" dashboard
# (uid aegis-forecast-drift-performance) into the governed Grafana folder via HTTP API.
#
# SECURITY:
...
#   - Refuses to publish if the Performance UID currently lives OUTSIDE the AEGIS folder.
#   - Idempotent: updates the same UID in place (no duplicate, no new dashboard).
#   - Touches ONLY the Performance dashboard. Does not modify the Overview, Forecast,
#     the other 8 section shells, datasources, nginx, Docker, CSVs, Python, Power BI,
#     alerts, or the token.

[CmdletBinding()]
param(
    [string]$DashboardsDir = (Join-Path $PSScriptRoot '..\grafana\dashboards'),
    [string]$Uid           = 'aegis-forecast-drift-performance',
    ...
    [string]$Message       = 'E7D.3 Performance MVP — analytical dashboard (avg perf drift score, computable count, avg MAPE change, coverage, drift trend, signal details, non-computable summary, run, DQ)'
)

$ErrorActionPreference = 'Stop'
function Info([string]$m) { Write-Host "[push-e7d3] $m" }
function Die([string]$m)  { Write-Host "[push-e7d3] ERROR: $m" -ForegroundColor Red; exit 1 }

if (-not (Test-Path -LiteralPath $TokenPath)) { Die "Encrypted token not found at $TokenPath." }
$file = Join-Path $DashboardsDir "$Uid.json"
if (-not (Test-Path -LiteralPath $file)) { Die "Performance dashboard file not found: $file" }
...
try {
    ...
    # --- 1. Conflict guard: Performance UID must not live in a foreign folder ---
