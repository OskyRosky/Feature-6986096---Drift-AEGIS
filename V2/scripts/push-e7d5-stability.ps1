# push-e7d5-stability.ps1
# E7D.5 - Publish ONLY the AEGIS Forecast Drift "Stability" dashboard
# (uid aegis-forecast-drift-stability) into the governed Grafana folder via HTTP API.
#
# SECURITY:
...
#   - Refuses to publish if the Stability UID currently lives OUTSIDE the AEGIS folder.
#   - Idempotent: updates the same UID in place (no duplicate, no new dashboard).
#   - Touches ONLY the Stability dashboard. Does not modify the Overview, Forecast,
#     Performance, Shape, the other 6 section shells, datasources, nginx, Docker, CSVs,
#     Python, Power BI, alerts, or the token.

[CmdletBinding()]
param(
    [string]$DashboardsDir = (Join-Path $PSScriptRoot '..\grafana\dashboards'),
    [string]$Uid           = 'aegis-forecast-drift-stability',
    ...
    [string]$Message       = 'E7D.5 Stability MVP — analytical dashboard (avg stability drift score, computable count, coverage, max score, drift trend, signal details, run, DQ)'
)

$ErrorActionPreference = 'Stop'
function Info([string]$m) { Write-Host "[push-e7d5] $m" }
function Die([string]$m)  { Write-Host "[push-e7d5] ERROR: $m" -ForegroundColor Red; exit 1 }

if (-not (Test-Path -LiteralPath $TokenPath)) { Die "Encrypted token not found at $TokenPath." }
$file = Join-Path $DashboardsDir "$Uid.json"
if (-not (Test-Path -LiteralPath $file)) { Die "Stability dashboard file not found: $file" }
...
try {
    ...
    # --- 1. Conflict guard: Stability UID must not live in a foreign folder ---
