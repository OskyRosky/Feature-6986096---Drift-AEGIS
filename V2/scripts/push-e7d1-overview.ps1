# push-e7d1-overview.ps1
# E7D.1 - Publish ONLY the AEGIS Forecast Drift "Overview" dashboard
# (uid aegis-forecast-drift-foundation) into the governed Grafana folder via HTTP API.
#
# SECURITY:
...
#   - Refuses to publish if the Overview UID currently lives OUTSIDE the AEGIS folder.
#   - Idempotent: updates the same UID in place (no duplicate, no new dashboard).
#   - Touches ONLY the Overview. Does not modify the 10 section shells, datasources,
#     nginx, Docker, CSVs, Python, Power BI, alerts, or the token.

[CmdletBinding()]
param(
    [string]$DashboardsDir = (Join-Path $PSScriptRoot '..\grafana\dashboards'),
    [string]$Uid           = 'aegis-forecast-drift-foundation',
    ...
    [string]$Message       = 'E7D.1 Overview MVP — analytical dashboard (KPIs, distribution, trend, family, key risk, run, DQ)'
)

$ErrorActionPreference = 'Stop'
function Info([string]$m) { Write-Host "[push-e7d1] $m" }
function Die([string]$m)  { Write-Host "[push-e7d1] ERROR: $m" -ForegroundColor Red; exit 1 }

if (-not (Test-Path -LiteralPath $TokenPath)) { Die "Encrypted token not found at $TokenPath." }
$file = Join-Path $DashboardsDir "$Uid.json"
if (-not (Test-Path -LiteralPath $file)) { Die "Overview dashboard file not found: $file" }
...
try {
    ...
    # --- 1. Conflict guard: Overview UID must not live in a foreign folder ---
